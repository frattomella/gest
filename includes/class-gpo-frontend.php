...
class GPO_Frontend {
    const CATALOG_FILTER_VALUES_TRANSIENT = 'gpo_catalog_filter_values_v2';
    const PUBLIC_VEHICLE_IDS_TRANSIENT = 'gpo_public_vehicle_ids_v1';
    ...
    protected static $gallery_items_cache = [];
    protected static $catalog_filter_values_cache = null;
    protected static $public_vehicle_ids_cache = null;
    ...
    protected static $legacy_featured_ids = null;
    public static function init() {
        ...
        add_filter('posts_clauses', [__CLASS__, 'catalog_registration_order_clauses'], 10, 2);
    }
    ...
    public static function display_settings() {
        if (self::$display_settings_cache !== null) {
            return self::$display_settings_cache;
        }
        ...
        self::$display_settings_cache = $settings;
        return self::$display_settings_cache;
    }
    ...
    public static function is_vehicle_publicly_available($post_id) {
        $post_id = absint($post_id);
        if ($post_id < 1) {
            return false;
        }
        if (array_key_exists($post_id, self::$vehicle_availability)) {
            return self::$vehicle_availability[$post_id];
        }

        $post = get_post($post_id);
        if (!$post || $post->post_type !== 'gpo_vehicle' || $post->post_status !== 'publish') {
            self::$vehicle_availability[$post_id] = false;
            return false;
        }

        if (get_post_meta($post_id, '_gpo_is_template_demo', true) === '1') {
            self::$vehicle_availability[$post_id] = false;
            return false;
        }

        $status = strtolower(remove_accents(trim((string) get_post_meta($post->ID, '_gpo_status', true))));
        if ($status === '') {
            self::$vehicle_availability[$post_id] = true;
            return true;
        }

        foreach (['non disponibil', 'not available', 'unavailable', 'vendut', 'sold', 'ritirat', 'withdrawn', 'rimoss', 'removed', 'eliminat', 'deleted', 'archiviat', 'archived', 'inattiv'] as $blocked_status) {
            if (strpos($status, $blocked_status) !== false) {
                self::$vehicle_availability[$post_id] = false;
                return false;
            }
        }

        self::$vehicle_availability[$post_id] = !in_array($status, ['0', 'no', 'false'], true);
        return self::$vehicle_availability[$post_id];
    }

    public static function filter_valid_vehicle_ids($ids, $limit = 0) {
        $ids = array_values(array_unique(array_filter(array_map('absint', (array) $ids))));
        if (!empty($ids) && function_exists('update_meta_cache')) {
            $preload_limit = $limit > 0 ? max(12, absint($limit) * 2) : 50;
            update_meta_cache('post', array_slice($ids, 0, $preload_limit));
        }

        $valid_ids = [];
        foreach ($ids as $post_id) {
            if (!self::is_vehicle_publicly_available($post_id)) {
                continue;
            }
            $valid_ids[$post_id] = $post_id;
            if ($limit > 0 && count($valid_ids) >= absint($limit)) {
                break;
            }
        }
    ...
    public static function get_platform_showcase_vehicle_ids($limit = 0) {
        if (self::$platform_showcase_ids === null) {
            ...
            $stored_ids = $snapshot_ready ? get_option('gpo_platform_showcase_vehicle_ids', null) : null;

            if (is_array($stored_ids)) {
                self::$platform_showcase_ids = self::parse_vehicle_ids($stored_ids);
            } else {
                ...
                $query = new WP_Query([
                    ...
                    'post_status' => 'publish',
                    'post__in' => self::public_vehicle_query_ids(),
                    'posts_per_page' => -1,
                    'fields' => 'ids',
                    'no_found_rows' => true,
                    'orderby' => 'post__in',
                ...
                self::$platform_showcase_ids = self::sort_vehicle_ids_by_recency(
                    self::filter_valid_vehicle_ids($query->posts)
                );

                if ($snapshot_ready) {
                    update_option('gpo_platform_showcase_vehicle_ids', self::$platform_showcase_ids, false);
                }
            }
        }
    ...
    public static function get_manual_extra_vehicle_ids($limit = 0) {
        if (self::$manual_showcase_ids === null) {
            ...
            $legacy_ids = $platform_auto_mode ? [] : self::get_current_featured_ids(100);
    ...
    public static function get_effective_showcase_vehicle_ids($limit = 0, $additional_ids = []) {
        ...
        if (self::$effective_showcase_ids === null) {
            self::$effective_showcase_ids = self::parse_vehicle_ids(array_merge(
                self::get_platform_showcase_vehicle_ids(),
                self::sort_vehicle_ids_by_recency(self::get_manual_extra_vehicle_ids())
            ));
        }

        $ids = empty($additional_ids)
            ? self::$effective_showcase_ids
            : self::parse_vehicle_ids(array_merge(
                self::$effective_showcase_ids,
                self::sort_vehicle_ids_by_recency($additional_ids)
            ));
    ...
    protected static function whatsapp_chat_url($post_id = 0) {
        ...
        $post_id = absint($post_id);
        $valid_vehicle = $post_id && self::is_vehicle_publicly_available($post_id);
        $title = $valid_vehicle ? trim(wp_strip_all_tags(get_the_title($post_id))) : 'questo veicolo';
        $url = $valid_vehicle ? get_permalink($post_id) : '';
    ...
    public static function handle_vehicle_lead_submission() {
        ...
        $vehicle_url = get_permalink($post_id);
    ...
    public static function vehicle_grid_shortcode($atts) {
        ...
        $rendered_vehicle = false;
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                if (self::render_card(get_the_ID(), $display)) {
                    $rendered_vehicle = true;
                }
            }
        }
        if (!$rendered_vehicle) {
    ...
    public static function share_actions_markup($post_id) {
        $post_id = absint($post_id);
        $url = self::is_vehicle_publicly_available($post_id) ? get_permalink($post_id) : '';
        if (!$url) {
            return '';
        }
    ...
    public static function single_share_modal_markup($post_id, $modal_id = '') {
        ...
        $html .= '<div class="gpo-share-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="' . esc_attr($modal_id . '-title') . '" tabindex="-1">';
        $html .= '<div class="gpo-share-modal__head"><h3 id="' . esc_attr($modal_id . '-title') . '">Condividi</h3></div>';
        $html .= '<div class="gpo-share-modal__actions">';
        $html .= '<button type="button" class="gpo-share-modal__action" data-gpo-copy-link="' . esc_url($url) . '" data-gpo-copy-label="Link copiato" data-gpo-share-dismiss="1"><span class="gpo-share-modal__action-icon" aria-hidden="true">' . self::icon_markup('copy') . '</span><span>Copia link</span></button>';
        $html .= '<a class="gpo-share-modal__action" href="' . esc_url($whatsapp_url) . '" target="_blank" rel="noopener noreferrer" data-gpo-share-dismiss="1"><span class="gpo-share-modal__action-icon" aria-hidden="true">' . self::icon_markup('whatsapp') . '</span><span>WhatsApp</span></a>';
        $html .= '<a class="gpo-share-modal__action" href="' . esc_url($facebook_url) . '" target="_blank" rel="noopener noreferrer" data-gpo-share-dismiss="1"><span class="gpo-share-modal__action-icon" aria-hidden="true">' . self::icon_markup('facebook') . '</span><span>Facebook</span></a>';
        $html .= '</div></div></div>';

        return $html;
    }

    protected static function print_text_preview($content, $words = 60) {
        $content = trim(preg_replace('/\s+/', ' ', wp_strip_all_tags((string) $content)));
        if ($content === '') {
            return '';
        }

        return wp_trim_words($content, max(12, absint($words)), '...');
    }

    protected static function print_list_preview($items, $limit = 12) {
        $items = array_values(array_filter(array_map('trim', (array) $items)));
        if (empty($items)) {
            return [];
        }

        $limit = max(4, absint($limit));
        if (count($items) <= $limit) {
            return $items;
        }

        $preview = array_slice($items, 0, $limit);
        $preview[] = '+' . (count($items) - $limit) . ' altri accessori disponibili su richiesta';
        return $preview;
    }

    protected static function site_social_profiles() {
        $profiles = [];
        $definitions = [
            'facebook' => ['facebook_url', 'social_facebook', 'facebook', 'facebook_link'],
            'instagram' => ['instagram_url', 'social_instagram', 'instagram', 'instagram_link'],
            'linkedin' => ['linkedin_url', 'social_linkedin', 'linkedin', 'linkedin_link'],
            'youtube' => ['youtube_url', 'social_youtube', 'youtube', 'youtube_link'],
        ];

        foreach ($definitions as $platform => $keys) {
            $value = '';
            foreach ($keys as $key) {
                $candidate = get_theme_mod($key, '');
                if (!is_string($candidate) || trim($candidate) === '') {
                    $candidate = get_option($key, '');
                }
    ...
    public static function print_sheet_markup($post_id, $data = []) {
        ...
        $html .= $logo_url
            ? '<img data-gpo-print-src="' . esc_url($logo_url) . '" alt="' . esc_attr($site_name) . '" />'
        ...
        if (!empty($social_profiles)) {
            ...
            foreach (array_keys($social_profiles) as $platform) {
                $html .= '<span class="gpo-print-sheet__social" aria-hidden="true">' . self::icon_markup($platform) . '</span>';
            }
            $html .= '</div>';
        }
        $html .= '</div></header>';
        ...
        if (is_array($main_photo)) {
            $html .= '<figure class="gpo-print-sheet__hero-photo"><img data-gpo-print-src="' . esc_url($main_photo['large']) . '" alt="' . esc_attr($main_photo['alt']) . '" /></figure>';
        }
        if ($gallery_count > 0) {
        ...
        if (!empty($additional_photos)) {
            ...
            foreach ($additional_photos as $index => $item) {
                ...
                $html .= '<figure class="gpo-print-sheet__photo' . ($is_more_tile ? ' gpo-print-sheet__photo--more' : '') . '"><img data-gpo-print-src="' . esc_url(!empty($item['thumb']) ? $item['thumb'] : $item['large']) . '" alt="' . esc_attr($item['alt']) . '" />';
                if ($is_more_tile) {
    ...
    protected static function build_vehicle_query($atts = []) {
        ...
        $use_registration_order = (($atts['filters'] ?? 'no') === 'yes')
            && (($sort === '' && $orderby === 'date') || in_array($sort, ['date_desc', 'year_desc'], true));
        $args = [
            'post_type' => 'gpo_vehicle',
            'post_status' => 'publish',
            'post__in' => self::public_vehicle_query_ids(),
    ...
    public static function catalog_registration_order_clauses($clauses, $query) {
        if (!$query instanceof WP_Query || !$query->get('gpo_catalog_registration_order')) {
            return $clauses;
        }

        global $wpdb;
        $clauses['join'] .= $wpdb->prepare(
            " LEFT JOIN {$wpdb->postmeta} AS gpo_registration_date_pm ON ({$wpdb->posts}.ID = gpo_registration_date_pm.post_id AND gpo_registration_date_pm.meta_key = %s)",
            '_gpo_registration_date'
        );
        $clauses['join'] .= $wpdb->prepare(
            " LEFT JOIN {$wpdb->postmeta} AS gpo_registration_year_pm ON ({$wpdb->posts}.ID = gpo_registration_year_pm.post_id AND gpo_registration_year_pm.meta_key = %s)",
            '_gpo_year'
        );
        $clauses['join'] .= $wpdb->prepare(
            " LEFT JOIN {$wpdb->postmeta} AS gpo_arrival_date_pm ON ({$wpdb->posts}.ID = gpo_arrival_date_pm.post_id AND gpo_arrival_date_pm.meta_key = %s)",
            '_gpo_arrival_date'
        );

        $registration_sort = "CASE WHEN gpo_registration_date_pm.meta_value REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' THEN LEFT(gpo_registration_date_pm.meta_value, 10) WHEN gpo_registration_year_pm.meta_value REGEXP '^[0-9]{4}$' THEN CONCAT(gpo_registration_year_pm.meta_value, '-01-01') ELSE NULL END";
        $arrival_sort = "CASE WHEN gpo_arrival_date_pm.meta_value REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}' THEN LEFT(gpo_arrival_date_pm.meta_value, 10) ELSE NULL END";
        $clauses['orderby'] = "({$registration_sort} IS NULL) ASC, {$registration_sort} DESC, ({$arrival_sort} IS NULL) ASC, {$arrival_sort} DESC, {$wpdb->posts}.post_date DESC, {$wpdb->posts}.ID DESC";
    ...
    protected static function publicly_available_vehicle_meta_clause() {
        return [
            'relation' => 'OR',
            [
                'key' => '_gpo_status',
    ...
    public static function invalidate_catalog_filter_cache() {
        ...
        self::$public_vehicle_ids_cache = null;
        self::$vehicle_availability = [];
        self::$vehicle_data_cache = [];
        self::$vehicle_card_data_cache = [];
        self::$gallery_items_cache = [];

        if (!self::$catalog_filter_cache_invalidated) {
            delete_transient(self::CATALOG_FILTER_VALUES_TRANSIENT);
            delete_transient(self::PUBLIC_VEHICLE_IDS_TRANSIENT);
            delete_transient('gpo_catalog_filter_values_v1');
            self::$catalog_filter_cache_invalidated = true;
        }
    }

    public static function rebuild_frontend_caches() {
        self::invalidate_catalog_filter_cache();
        self::public_vehicle_ids(true);
        self::catalog_filter_values(true);
    }

    protected static function public_vehicle_ids($force_refresh = false) {
        if (!$force_refresh && self::$public_vehicle_ids_cache !== null) {
            return self::$public_vehicle_ids_cache;
        }

        if (!$force_refresh) {
            $cached = get_transient(self::PUBLIC_VEHICLE_IDS_TRANSIENT);
            if (is_array($cached)) {
                self::$public_vehicle_ids_cache = self::parse_vehicle_ids($cached);
                return self::$public_vehicle_ids_cache;
            }
        }
        ...
        self::$public_vehicle_ids_cache = self::sort_vehicle_ids_by_recency($ids);
        set_transient(self::PUBLIC_VEHICLE_IDS_TRANSIENT, self::$public_vehicle_ids_cache, DAY_IN_SECONDS);
        return self::$public_vehicle_ids_cache;
    }

    protected static function public_vehicle_query_ids() {
        $ids = self::public_vehicle_ids();
        return !empty($ids) ? $ids : [0];
    }

    protected static function catalog_filter_values($force_refresh = false) {
        if (!$force_refresh && self::$catalog_filter_values_cache !== null) {
        ...
        $cached = $force_refresh ? false : get_transient(self::CATALOG_FILTER_VALUES_TRANSIENT);
        ...
        $public_ids = self::public_vehicle_ids();
        if (empty($public_ids)) {
            self::$catalog_filter_values_cache = array_fill_keys(array_keys($fields), []);
            set_transient(self::CATALOG_FILTER_VALUES_TRANSIENT, self::$catalog_filter_values_cache, DAY_IN_SECONDS);
            return self::$catalog_filter_values_cache;
        }
        ...
        $id_placeholders = implode(', ', array_fill(0, count($public_ids), '%d'));
        $rows = $wpdb->get_results($wpdb->prepare(
            ...
            ...array_merge($meta_keys, $public_ids)
        ), ARRAY_A);
    ...
    protected static function rotating_vehicle_id($ids, $strategy = 'auto') {
        $ids = self::parse_vehicle_ids($ids);
        $count = count($ids);
        if ($count < 1) {
            return 0;
        }
        ...
        $start_index = $strategy === 'manual'
            ? $bucket % $count
            : ((int) sprintf('%u', crc32(implode('|', $ids) . '|' . $bucket))) % $count;

        for ($offset = 0; $offset < $count; $offset++) {
            $candidate_id = absint($ids[($start_index + $offset) % $count]);
            if (self::is_vehicle_publicly_available($candidate_id)) {
                return $candidate_id;
            }
        }
    ...
    protected static function related_brand_vehicle_ids($post_id, $limit = 6) {
        ...
        $brand = self::is_vehicle_publicly_available($post_id)
            ? trim((string) get_post_meta($post_id, '_gpo_brand', true))
            : '';
        ...
        $query = new WP_Query([
            ...
            'no_found_rows' => true,
            'orderby' => 'post__in',
            'meta_query' => [self::brand_meta_query($brand)],
        ]);

        $ids = self::filter_valid_vehicle_ids($query->posts, $limit);
        if (count($ids) >= $limit) {
    ...
    protected static function get_current_featured_ids($limit = 12) {
        if (self::$legacy_featured_ids === null) {
            $query = new WP_Query([
                'post_type' => 'gpo_vehicle',
                'post_status' => 'publish',
                'post__in' => self::public_vehicle_query_ids(),
            ...
            self::$legacy_featured_ids = [];
            foreach ($query->posts as $post_id) {
                if (self::is_vehicle_publicly_available($post_id) && self::is_currently_featured($post_id)) {
                    self::$legacy_featured_ids[] = absint($post_id);
                }
            }
        }

        return array_slice(self::$legacy_featured_ids, 0, max(1, absint($limit)));
    }

    protected static function latest_real_vehicle_ids($limit = 12) {
        return array_slice(self::public_vehicle_ids(), 0, max(1, absint($limit)));
    }
    ...
    public static function template_preview_vehicle_link($template_id = 0) {
        ...
        if (!$vehicle_id || !self::is_vehicle_publicly_available($vehicle_id)) {
            return '';
        }
    ...
    protected static function gallery_items($post_id) {
        $post_id = absint($post_id);
        if (array_key_exists($post_id, self::$gallery_items_cache)) {
            return self::$gallery_items_cache[$post_id];
        }
        ...
        self::$gallery_items_cache[$post_id] = $items;
        return self::$gallery_items_cache[$post_id];
    }
    ...
    public static function gallery_markup($post_id, $with_panel = false) {
        ...
        if (!empty($items)) {
            ...
            echo '<img class="gpo-gallery-lightbox__image" data-gpo-lightbox-src="' . esc_url($current['full']) . '" alt="' . esc_attr($current['alt']) . '" decoding="async" />';
...
/', (string) get_post_meta($post_id, $meta_key, true))));
    ...
    protected static function icon_markup($type) {
        ...
        if ($type === 'instagram') {
            return '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="5"></rect><circle cx="12" cy="12" r="4"></circle><circle cx="17.5" cy="6.5" r="1"></circle></svg>';
        }

        if ($type === 'linkedin') {
            return '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M6.94 8.5H3.56V20h3.38V8.5ZM5.25 3A1.97 1.97 0 0 0 3.3 4.97c0 1.08.86 1.97 1.93 1.97h.02A1.97 1.97 0 0 0 7.2 4.97 1.96 1.96 0 0 0 5.25 3ZM20.7 13.02c0-3.38-1.8-4.95-4.21-4.95-1.94 0-2.8 1.08-3.29 1.83V8.5H9.82c.04.92 0 11.5 0 11.5h3.38v-6.42c0-.34.02-.68.12-.92.27-.68.88-1.39 1.9-1.39 1.34 0 1.88 1.05 1.88 2.58V20H20.7v-6.98Z"/></svg>';
        }

        if ($type === 'youtube') {
            return '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M21.6 7.2a2.95 2.95 0 0 0-2.08-2.08C17.7 4.6 12 4.6 12 4.6s-5.7 0-7.52.52A2.95 2.95 0 0 0 2.4 7.2 30.7 30.7 0 0 0 1.9 12c0 1.62.17 3.23.5 4.8a2.95 2.95 0 0 0 2.08 2.08C6.3 19.4 12 19.4 12 19.4s5.7 0 7.52-.52a2.95 2.95 0 0 0 2.08-2.08c.33-1.57.5-3.18.5-4.8 0-1.62-.17-3.23-.5-4.8ZM9.9 15.06V8.94L15.2 12l-5.3 3.06Z"/></svg>';
        }

        if ($type === 'globe') {
            return '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"></circle><path d="M3 12h18"></path><path d="M12 3a15 15 0 0 1 0 18"></path><path d="M12 3a15 15 0 0 0 0 18"></path></svg>';
        }
    ...
    public static function ajax_live_search() {
        ...
        while ($query->have_posts()) {
            $query->the_post();
            $id = get_the_ID();
            if (!self::is_vehicle_publicly_available($id)) {
                continue;
            }
